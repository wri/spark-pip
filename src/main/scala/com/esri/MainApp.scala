...
object MainApp extends App {
  ...
  case class PointsRow(polyid: String, iso: String, id1: String, id2: String, year: String, area: Double, thresh: Long, biomass: Double)
  ...
  with_poly.map({case Array(lon, lat, year, area, thresh, biomass, polyid, iso, id1, id2) => (PointsRow(polyid, iso, id1, id2, year, area.toDouble, matchTest(thresh), biomass_per_pixel(biomass)(area))) })
           .toDF()
           .groupBy("polyid", "iso", "id1", "id2", "thresh", "year").agg(sum("area"), sum("biomass"))
           .write
           .format("csv")
           .save("output")
